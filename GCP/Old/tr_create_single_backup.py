...
print("Syntax is: python tr_create_single.py image zone machine-size subnetwork machine-type  'work-hours-only(yes/no)' 'do not stop(yes/no)' 'do not delete(yes/no)'   ")
print("Example: python tr_create_single.py grc-new2 us-east1-b n1-standard-2 rnd-subnet single  yes yes no")

image = sys.argv[1]
zone = sys.argv[2]
machine_size = sys.argv[3]
subnetwork = sys.argv[4]
machine_type = sys.argv[5]
work_hours_only = sys.argv[6]
do_not_stop_tag = sys.argv[7]
do_not_delete_tag = sys.argv[8]
#zone = us-east1-b
region = zone[:-2]
#region = "us-east1"
...
if work_hours_only != 'yes':
    if work_hours_only != 'no':
        print("You must only type 'yes' or 'no' at the 'work_hours_only' field ")
        sys.exit()
...
print("You selected: " + "\nImage: " + image + "\nZone: " + zone + "\nMachine Size: " + machine_size
 + "\nMachine Type: " + machine_type + "\nSubnetwork: " + subnetwork +"\nWork Hours Only: " + work_hours_only
...
print("Adding working hours only tag")
if work_hours_only == "no":
    work_hours_only = "work-hours-only-no"
    subprocess.check_call(['gcloud', 'compute', 'instances', 'add-tags', instance_name, '--tags', work_hours_only])
if work_hours_only == "yes":
    work_hours_only = "work-hours-only-yes"
    subprocess.check_call(['gcloud', 'compute', 'instances', 'add-tags', instance_name, '--tags', work_hours_only])
